...
[
  ['チームを支え、仲間を大切にするリーダー', 2, 1, 0, 0],
  ['明確な目標に向かって粘り強く進むリーダー', 0, 3, -1, 0],
  ['リスクを恐れず、新しい挑戦を推奨するリーダー', 0, 0, 1, 2],
  ['自由で柔軟な発想を持ち、規則に縛られないリーダー', -1, -1, 2, 1]
].each do |info|
  content = info[0]
  question.options.create!(
    content: content,
...
[
  ['他人の気持ちを癒す「愛の力」', 3, 0, 0, -1],
  ['どんな問題も計画的に解決する「超思考力」', 0, 3, 0, 0],
  ['未知の場所に瞬時に移動できる「瞬間移動」', 0, 0, 1, 2]
].each do |info|
  content = info[0]
  question.options.create!(
    content: content,
...
[
  ['家族や友達が集まる、賑やかな場所', 3, 0, 0, 0],
  ['きっちりと整理された、効率的な空間', 0, 3, -1, 0],
  ['一人の時間を楽しめる静かな空間', -1, 0, 3, 0],
  ['自分の思い通りに改造できる基地', 0, 0, 0, 3]
].each do |info|
  content = info[0]
  question.options.create!(
    content: content,
...
[
  ['相談できる人がいない', 3, 0, 0, 0],
  ['常に周りに合わせないといけない', -1, 0, 2, 1],
  ['やることがなくてめっちゃ暇', 0, 2, 0, 2],
  ['忙しすぎて時間がない', 0, -1, 3, 0]
].each do |info|
  content = info[0]
  question.options.create!(
